...
from config.token_usage_config import TokenUsageConfig
from config.acp_config import load_acp_config_from_dict
from config.checkpointer_config import CheckpointerConfig, load_checkpointer_config_from_dict
...
from config.title_config import TitleConfig, load_title_config_from_dict
from config.token_usage_config import TokenUsageConfig
from config.tool_config import ToolConfig, ToolGroupConfig
from config.tool_search_config import ToolSearchConfig, load_tool_search_config_from_dict
...
class AppConfig(BaseModel):
    ...
    prompt_caching: PromptCachingConfig = Field(default_factory=PromptCachingConfig, description="Prompt/context caching configuration")
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path:
        ...
        3. 否则，从 `_default_config_candidates()` 中查找确定性的后端/仓库根目录默认值。
        """
        if config_path:
    ...
    def from_file(cls, config_path: str | None = None) -> Self:
        ...
        # Load prompt caching config, resetting to defaults when omitted.
        load_prompt_caching_config_from_dict(config_data.get("prompt_caching", {}))
        ...
        if isinstance(config, str):
            if config.startswith("$"):
                ...
                if env_value is None:
                    logger.warning("Environment variable %s not found for config value %s; leaving placeholder unchanged", config[1:], config)
                    return config
                return env_value
            return config
...
def _load_and_cache_app_config(config_path: str | None = None) -> AppConfig:
    ...
    # 1，找文件路径 -> 2，加载配置实例 -> 3，更新全局缓存变量（实例、路径、修改时间、是否为自定义）
...
def get_app_config() -> AppConfig:
    ...
    the cache.
    获取 DeerFlow 配置实例。
    返回一个缓存的单例实例，并在底层配置文件路径或修改时间发生变化时自动重新加载。
    使用 `reload_app_config()` 强制重新加载，或使用 `reset_app_config()` 清除缓存。
    """
...
def reload_app_config(config_path: str | None = None) -> AppConfig:
    ...
    - 重新从文件加载配置并更新缓存的实例。
    - 当配置文件已被修改并且您想在不重启应用程序的情况下获取更改时，这非常有用。
    Args:

...
class MemoryConfig(BaseModel):
    ...
    storage_path: str = Field(
        ...
        description=(
            ...
            "Note: if you previously set this to `.agentflow/memory.json`, "
            "the file will now be resolved as `{base_dir}/.agentflow/memory.json`; "
            "migrate existing data or use an absolute path to preserve the old location."
        ),
    )
    storage_class: str = Field(
        default="agents.memory.storage.FileMemoryStorage",

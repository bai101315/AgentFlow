...
from agents.thread_state import SandboxState, ThreadDataState, ThreadState
...
"""
ThreadPoolExecutor: 创建一个专门给子代理（subagent）用的后台线程池，用来异步跑子代理任务，不阻塞主 Agent
创建一个最多同时跑 3 个子代理、线程名字带 subagent 标识的后台任务池，是子代理能异步、稳定、安全运行的底层核心
"""
...
class SubagentExecutor:
    ...
    def _create_agent(self):
        ...
        from agents.middlewares.tool_error_handling_middleware import build_subagent_runtime_middlewares
    ...
    def execute(self, task: str, result_holder: SubagentResult | None = None) -> SubagentResult:
        ...
        """
        # 解决痛点：1， 同步代码调用异步库（比如这里的 MCP 工具），async def 是跑不起来的，需要 asyncio.run() 来启动
        # 2, 本身已经在一个异步程序里了（比如父 Agent 正在跑异步循环），你不能在里面直接调用 asyncio.run() 去跑另一个异步任务，会报 “循环已在运行” 的错误。
    ...
    def _execute_in_isolated_loop(self, task: str, result_holder: SubagentResult | None = None) -> SubagentResult:
        ...
        """
        # 在全新的事件循环中执行子代理。
        # 此方法旨在在单独的线程中运行，以确保与任何父事件循环完全隔离，
        try:
            ...
            try:
                ...
                if pending:
                    ...
                    for task_obj in pending:
                        task_obj.cancel()
                    # 2. 等待这些任务响应取消（处理 CancelledError）
                ...
                try:
                    ...
                    # 我们把函数最开头保存的那个 previous_loop 重新设为当前线程的 loop。
    ...
    async def _aexecute(self, task: str, result_holder: SubagentResult | None = None) -> SubagentResult:
        ...
        """
        # task: sub-agent的任务描述
        # result_holder：可选的预先创建的结果对象，用于在执行期间更新。
        ...
        try:
            ...
            context = {}
            if self.thread_id:
                # 如果有 thread_id，说明要“带记忆聊天”，把 ID 传进去
            ...
            async for chunk in agent.astream(state, config=run_config, context=context, stream_mode="values"):  # type: ignore[arg-type]
                ...
                if messages:
                    ...
                    if isinstance(last_message, AIMessage):
                        ...
                        # 去重逻辑
                        # 流式输出可能会把同一条消息吐好几次，我们要判断是不是已经存过了

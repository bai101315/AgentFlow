...
def _build_runtime_middlewares(
    *,
    ...
    # include_uploads=True： 是否包含与 “文件 / 数据上传” 相关的中间件
    # include_dangling_tool_call_patch=True： 是否包含修复 “悬空工具调用” 的补丁中间件；
    # agent调用可能会导致异常情况，改中间件会进行修补；1. 检测到悬空调用时，自动重试工具调用；
    # 2, 重试失败则返回友好提示; 3, 回滚到工具调用前的 Agent 状态，保证后续流程可继续；
    # lazy_init: bool = True: 控制中间件的初始化策略（懒加载 / 即时加载）
    ...
    if include_uploads:
        from agents.middlewares.uploads_middleware import UploadsMiddleware
    ...
    # NOTE: guardrails_config暂时不考虑
    # Guardrail middleware (if configured)
    #  Guardrails = Agent 的安全防护层（比如内容审核、指令白名单、权限拦截、危险操作过滤）
    # from config.guardrails_config import get_guardrails_config

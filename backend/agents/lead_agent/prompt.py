...
from config.agents_config import load_agent_soul
from skill import load_skills
from skill.types import Skill
from subagents import get_available_subagent_names
...
# 这是后台真正刷新缓存的函数，用版本号解决了 “刷新过程中缓存又失效了” 的竞态问题。
...
def get_skills_prompt_section(available_skills: set[str] | None = None) -> str:
    ...
    # print(f"skill_signature: {skill_signature}")
...
def get_deferred_tools_prompt_section() -> str:
    ...
    """
    # 生成<available-deferred-tools>, 列出已延迟的工具名称;可以使用 `tool_search` 加载它们
    # 如果 `tool_search` 被禁用或没有已延迟的工具，则返回空字符串。

    from tools.builtins.tool_search import get_deferred_registry
